...
public class ViNsoPostComment extends EntityAbstract<ViNsoPostComment> {
	...
	public		static 	final EntityDAO<ViNsoPostComment> 	DAO;
	...
	static{
		DAO = new EntityDAO<ViNsoPostComment>(Hnv_CfgHibernate.reqFactoryEMSession(Hnv_CfgHibernate.ID_FACT_MAIN) , ViNsoPostComment.class, RIGHTS);
	}
...
//	private	String          T_Property_04;
	...
	/*
	 * @Column(name=COL_T_AVATAR, nullable = true) private String T_Avatar;
	 * 
	 * @Column(name=COL_T_AVATAR_USER, nullable = true) private String
	 * T_Avatar_User;
	 * 
	 * @Column(name=COL_T_NAME_USER, nullable = true) private String T_Name_User;
	 * 
	 * @Column(name=COL_T_TYPE_AVATAR, nullable = true) private Integer
	 * T_Type_Avatar;
	 */
	//-----------------------Transient Variables-------------------------
	@Transient
	...
	@Transient
	private List<ViNsoPostComment> 		    O_Children;

	
	//---------------------Constructeurs-----------------------
	private ViNsoPostComment(){}

	public ViNsoPostComment(Map<String, Object> attrs) throws Exception {
	...
	@Override
	public void doMergeWith(ViNsoPostComment ent) {
		if (ent == this) return;
...
//		this.T_Entity_Title         = ent.T_Entity_Title;
	...
	public boolean equals(Object o)  {
		...
		ok = (I_ID == ((ViNsoPostComment)o).I_ID);
		if (!ok) return ok;
	...
	public static List<ViNsoPostComment> reqBuildChildren(List<ViNsoPostComment> list, Criterion criLev) throws Exception {
	    Set<Integer> ids = ToolSet.reqSetInt(list, ViNsoPostComment.ATT_I_ID);
	    if(ids!=null && ids.size()>0) {
	        List<ViNsoPostComment> childs = ViNsoPostComment.DAO.reqList_In(ViNsoPostComment.ATT_I_PARENT, ids, criLev);
	        ViNsoPostComment.doBuildListUsers(childs);
	        if(childs != null && childs.size() > 0) {
	            Map<Integer, List<ViNsoPostComment>> mapChild = new HashMap<Integer, List<ViNsoPostComment>>();

	            for(ViNsoPostComment post: childs) {
	                Integer parentID = (Integer) post.req(ViNsoPostComment.ATT_I_PARENT);
	                if(parentID != null) {
	                    if(mapChild.containsKey(parentID)) {
	                        mapChild.get(parentID).add(post);
	                    }else {
	                        List<ViNsoPostComment> ls = new ArrayList<ViNsoPostComment>();
	                        ls.add(post);
	            ...
	            for(ViNsoPostComment p: list) {
	                Integer postID = (Integer) p.req(ViNsoPostComment.ATT_I_ID);
	                p.reqSet(ViNsoPostComment.ATT_O_CHILDREN, mapChild.get(postID));
	            }
	        }
	        return list;
	    }
	    return new ArrayList<ViNsoPostComment>();
	}
	
	public static void doBuildListUsers(List<ViNsoPostComment> list) throws Exception{
...
//					+   " NULL" 													+ " AS " + COL_T_ENTITY_TITLE 		+ ","
	...
	private static final String SQL_REQ_WHERE_STAT 		= " a."			+ TaNsoPost.COL_I_STATUS_01 	+ " = %d";
	
	private static final String SQL_REQ_WHERE_ENTID 	= " a."			+ TaNsoPost.COL_I_VAL_01 	+ " = %d";
	
	private static final String SQL_REQ_WHERE_CAT 		=  " AND cat."	+ TaTpyCategoryEntity.COL_I_TPY_CATEGORY	+ " in %s "	;
	
	private static final String SQL_REQ_WHERE_STAT_NOT_EQUAL 	= " a."	+TaNsoPost.COL_I_STATUS_01 	+ " != %d";
	...
	//---------------------------------------------------------------------------------
	public static List<ViNsoPostComment> reqNsoPostLstByUserId(Integer userId) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
		String sql = reqNsoPostLstByUserIdSql(userId);		
		list = ViNsoPostComment.DAO.reqList(sql);
		return list;
	}

	//---------------------------------------------------------------------------------
	public static List<ViNsoPostComment> reqNsoPostLstByUserId(Integer userId, Integer begin, Integer nbRes) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
		String sql = reqNsoPostLstByUserIdSql(userId);		
		list = ViNsoPostComment.DAO.reqList(begin, nbRes, sql);
		return list;
	}
	...
	private static String reqNsoPostLstByUserIdSql(Integer userId) throws Exception {
		...
		sql += " AND " + String.format(SQL_REQ_WHERE_STAT_NOT_EQUAL, TaNsoPost.STAT_01_DELETED);


		String orderCol = ViNsoPostComment.ATT_D_DATE_02;
	...
	public static Integer reqNsoPostLstByUserCount(Integer userId) throws Exception {
		...
		Integer count = ViNsoPostComment.DAO.reqCount(sql).intValue();
	...
	//----------------------------------------------------------------------
	public static List<ViNsoPostComment> reqNsoPostLstThreadEvaluation(Date eval, Date now) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
		String sql = reqNsoPostLstGridSqlThreadEvaluation();		
		list = ViNsoPostComment.DAO.reqList(sql, COL_EVAL, eval, COL_NOW, now);
	...
	public static List<ViNsoPostComment> reqLstPost(Integer begin, Integer nbRes, Integer type, String multiType,  Integer stat, Integer style,  Set<Integer> catIds, String langId) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
...
//		}
	...
	public static void doBuildTranslations(List<ViNsoPostComment> list) throws Exception{
//		TranslationTool.doBuildTpyTranslations(list, DefDBExt.ID_TA_NSO_POST, ViNsoPost.ATT_O_TRANSLATIONS, false);
	...
	public static List<ViNsoPostComment> reqLstPostPrj(Integer begin, Integer nbRes, Integer type, String multiType,  Integer stat, Integer style,  Set<Integer> catIds, String langId, Integer entId) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
	...
	private static String reqNsoPostPrjSql(Integer type, String multiType, Integer stat, Integer style, Set<Integer> catIds, String langId, Integer entId) throws Exception {
		...
		if(entId != null)
			sql += " AND " + String.format(SQL_REQ_WHERE_ENTID, entId);
	...
	public static List<ViNsoPostComment> reqLstNewsPrj(Integer begin, Integer nbRes, Integer type, String multiType,  Integer stat, Integer style,  Set<Integer> catIds, String langId, Integer entId) throws Exception {
		List<ViNsoPostComment> 	list 	= new ArrayList<ViNsoPostComment>();		
