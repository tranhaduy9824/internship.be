...
* ----- Copyright 2023------------
*/
public class ServicePerSupplier implements IService {
	...
	public static final String SV_CLASS 					= "ServicePerSupplier".toLowerCase();	
	...
	public void doService(JSONObject json, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if(sv.equals(SV_LCK_REQ)					&& (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_PER_SUPPLIER_MOD)
					||	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckReq(user,  json, response);
			} else if(sv.equals(SV_LCK_SAV)				&& (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_PER_SUPPLIER_MOD)
					||	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckSav(user,  json, response);
			} else if(sv.equals(SV_LCK_END)				&& (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_PER_SUPPLIER_MOD)
					||	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckEnd(user,  json, response);
			} else if(sv.equals(SV_LCK_DEL)				&& (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_PER_SUPPLIER_MOD)
					||	APIAuth.canAuthorize(user, SV_CLASS, sv))){
			...
			} else if(sv.equals(SV_MOD) 				&& (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_PER_SUPPLIER_MOD)
					||	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doMod(user,  json, response);
			
			} else  if(sv.equals(SV_DEL)				&&  (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_DEL, APIAuth.R_PER_SUPPLIER_DEL) 
					|| 	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doDel(user,  json, response);
			
			
			} else if(sv.equals(SV_GET) 				&&  (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_PER_SUPPLIER_GET) 
					|| 	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doGet(user,  json, response);
							
			} else if(sv.equals(SV_LST)					&&  (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_PER_SUPPLIER_GET) 
					|| 	APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLst(user,  json, response);
				
				
			} else if(sv.equals(SV_LST_PAGE)			&&  (APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_PER_SUPPLIER_GET) 
					|| 	APIAuth.canAuthorize(user, SV_CLASS, sv))){
	...
	private static TaPerPerson reqNew(TaAutUser user,  JSONObject json) throws Exception {
		...
		//---Supplier must be a moral person
		ent.reqSet(TaPerPerson.ATT_I_TYPE_01, TaPerPerson.TYP_01_MORAL);
		ent.reqSet(TaPerPerson.ATT_I_TYPE_02, TaPerPerson.TYP_02_SUPPLIER);
	...
	private static TaPerPerson reqMod(TaAutUser user,  JSONObject json) throws Exception {
		...
		attr.put(TaPerPerson.ATT_I_TYPE_02, TaPerPerson.TYP_02_SUPPLIER);
